...
function is_git_repo()
    local bool = vim.api.nvim_eval(
                     'system("git rev-parse --is-inside-work-tree")')
    local parsed_bool = vim.fn.split(bool, '\n')[1]
    return parsed_bool
end

repo_name = vim.api.nvim_eval(
                'split(system("git rev-parse --show-toplevel"), "/")[-1]')
parsed_repo_name = vim.fn.split(repo_name, '\n')[1]
workbench_path = default_path .. parsed_repo_name

function directory_not_exist() return vim.fn.glob(workbench_path) == '' end

function workbench_file_path() return workbench_path .. "/" .. "workbench.md" end
...
function workbench.toggle()
    ...
    if is_git_repo() ~= "true" then
        return vim.api.nvim_err_writeln(
                   "Sorry, Workbench only works in a Git Repo")
    end

    -- Should not be here, but it will fix the problem i guess
    if directory_not_exist() then vim.fn.mkdir(workbench_path) end
    ...
    if buf_info then buf_hidden = buf_info.hidden end
